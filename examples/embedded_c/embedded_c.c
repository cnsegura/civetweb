...
/*NEED TO FIX THE ifdef stattements the define below is a hack*/
#define PORT "8888"
...
ExampleHandler(struct mg_connection *conn, void *cbdata)
{
	...
	mg_printf(conn, "table {border-collapse: collapse;}");
	mg_printf(conn, "th, td { padding: 0px;	text-align: left;}");
	mg_printf(conn, "h1, h2 { color:#0078D7;}");
	mg_printf(conn, "h3, h4, h5 { color:#505050;}");
	mg_printf(conn, "img { width: 400px; height: auto;}");
	mg_printf(conn, "</style>");
	mg_printf(conn, "<body>");
	mg_printf(conn, "<table>");
	mg_printf(conn, "<tr><td colspan='3'><img src='./WinAzure_logo.png'></td></tr>");
	mg_printf(conn, "<tr><td colspan='3'><h2>Configuration and management console</h2></td></tr>");
	mg_printf(conn, "<tr><td colspan='3' style='border-bottom-width: 0px; border-bottom-style: solid; border-top-width: 1px; border-top-style: solid;'><h4>Gateway configuration</h4></td></tr>");
	mg_printf(conn, "<tr><td>Things</td><td>Stuff</td><td>More stuff</td></tr>");
	mg_printf(conn, "<tr><td style='border-top-width: 1px; border-top-style: solid;'><h4>BACNet configuration</h4></td></tr>");
	mg_printf(conn, "<tr><td colspan='2'>BACNet Polling Interval </td>");
	mg_printf(conn, "<td><form action = 'action_page.php'><input type = 'range' name = 'points' min = '5' step = '5' max = '30'></td><td><button type = 'button'>Update</button></form></td></tr>");
	mg_printf(conn, "<tr><td style='border-bottom-width: 1px; border-bottom-style: solid;'>BACNet Device Discovery </td>");
	mg_printf(conn, "<td><button type = 'button' onclick = ''>Discover</button></td></tr>");
	mg_printf(conn, "</table>");

	mg_printf(conn, "<h3>This is an example text from a C handler</h3>");
	...
	mg_printf(conn, "<p>To see a page from the FileHandler handler <a href=\"form\">click form</a> (the starting point of the <b>form</b> test)</p>");
...
main(int argc, char *argv[])
{
	...
	if (ctx == NULL) {
		printf("stupid thing won't start\n");
	}
